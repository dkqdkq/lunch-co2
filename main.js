...
app.get('/api', async (req, res) => {
    ...
    const data = await axios.get(`https://open.neis.go.kr/hub/mealServiceDietInfo?ATPT_OFCDC_SC_CODE=B10&SD_SCHUL_CODE=${SchoolCode}&Type=json&MLSV_YMD=${YYYYMMDD}`);
...
app.get('/menu', async (req, res) => {
    const query = req.query;
    const headers = {'User-Agent': 'Mozilla/5.0 (Macintosh; Intel Mac OS X 12_6) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/117.0.0.0 Safari/537.36'};
    const YYYYMMDD = 0;
    const mlsvId = 2376928;
    const data = await axios.get(`https://yangcheon.sen.hs.kr/dggb/module/mlsv/selectMlsvDetailPopup.do?mlsvId=${mlsvId}`, { headers } );
